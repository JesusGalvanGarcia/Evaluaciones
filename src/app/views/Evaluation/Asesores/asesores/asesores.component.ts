...
export class AsesoresComponent implements OnInit {
  ...
  toggleRow(row: any) {
    ...
    const posicion = this.ListColaborator.findIndex(
      (elemento) => elemento.user_evaluation_id === row.user_evaluation_id
    );
  ...
  getTable() {
    ...
    this.userEvaluationService
      .GetColaboradorEvaluationsWithParams(data)
      .then((response: any) => {
    ...
    switch (process) {
      ...
      case 'Evaluación de Competencias':
        if (status == 'Terminado') {
          ...
          const elemento = detalle.find(
            (item: any) => item.name === 'Evaluación de Desempeño'
          );

          if (elemento.status != 'Terminado')
            this.message.error(
              'La evaluación de desempeño no se ha terminado de contestar.'
            );
          else this.router.navigate(['/competencias/' + id]);
        }
        break;
    }
  }
