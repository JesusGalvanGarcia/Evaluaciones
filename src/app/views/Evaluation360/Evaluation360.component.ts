...
export class Evaluation360Component implements OnInit {
  ...
  getPreview()
  {
    ...
    this.noteUser = {
      ...
      this.userTestService.getPreview(this.noteUser)
        .then((response: any) => {
        ...
        .catch((error: any) => {
          console.error('Error in the request:', error);
          this.message.error('Los datos no se obtuvieron de forma correcta. ' + error);
          //this.indexQuestion = this.indexQuestion - 1;
          // Handle errors here
        });
  ...
  PostsaveSuggestions(form: NgForm) {
    ...
    this.userTestService.SendTestSuggestions(this.suggestions)
      .then((response: any) => {
  ...
  PostsaveNote(idModule: number) {
    ...
    this.userTestService.SendAverage(this.noteUser)
      .then((response: any) => {
