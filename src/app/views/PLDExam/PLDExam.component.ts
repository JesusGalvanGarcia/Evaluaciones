...
@Component({
  ...
  imports: [MatMenuModule,CommonModule,MatIconModule,MatCardModule,LoadingComponent,GridModule,MatBadgeModule,MatProgressBarModule]
})
export class PLDExamComponent implements OnInit {
 ...
 attempt:number=0;
  constructor(private router: Router,private evaluation:EvaluationService,private route: ActivatedRoute, private pldService: PLDUserService, public message: MensajeService) 
  { 
  ...
  getExam(data: any) 
  {
    ...
    this.evaluation.GetExam(data, this.user_test_id)
      .then((response: any) => {
       
        this.PLDTest = response.data.test;//Obtener la informacion y desordenar las preguntas
        this.PLDTest.test_modules[0].questions=this.shuffleArray(this.PLDTest.test_modules[0].questions);
  ...
  PostsaveAnswers(question: Question, answer: Answer,idAnswer:number) {
    ...
    this.pldService.SendTestPLD(this.saveIndivisual)
      .then((response: any) => {
        ...
        this.nextQuestion(question, answer,idAnswer,response.actual_score,response); //pasar a la siguiente pregunta
      })
  ...
  //Actualizar la pregunta en  el array
   this.PLDTest.test_modules[0].questions[this.index].answers.map((respuesta) => {
     respuesta.user_answer_id=null;
 });
 this.PLDTest.test_modules[0].questions[this.index].answers[idAnswer].user_answer_id=answer.id;
